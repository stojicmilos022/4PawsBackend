...
// prikaz autora
function loadMasteri() {
	document.getElementById("data").style.display = "block";
	document.getElementById("loginForm").style.display = "none";
	document.getElementById("registerForm").style.display = "none";

	// ucitavanje autora
	var requestUrl = host + port + masterEndpoint;
	console.log("URL zahteva: " + requestUrl);
	var headers = {};
	if (jwt_token) {
		headers.Authorization = 'Bearer ' + jwt_token;			// headers.Authorization = 'Bearer ' + sessionStorage.getItem(data.token);
	...
	if(Object.keys(headers).length === 0)
	{
		...
		.then((response) => {
			if (response.status === 200) {
				response.json().then(setMasterUnAuthorised);
			} else {
				console.log("Error occured with code " + response.status);
				showError();
			}
...
function setMasterAuthorised(data) {
	var container = document.getElementById("data");
	container.innerHTML = "";
...
function setMasterUnAuthorised(data) {
	var container = document.getElementById("data");
	container.innerHTML = "";
...
function createHeader() {
	...
	row.appendChild(createTableCell("Naziv"));
	row.appendChild(createTableCell("Slave"));
	row.appendChild(createTableCell("Godina"));
